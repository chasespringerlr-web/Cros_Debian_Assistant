...
def load_board_catalog(force=False):
    ...
    if pio:
        try:
            ...
            pass
    dedup={}
    for b in catalog:        key=_compact_text(b.get("name"))
        if not key:
            continue
...
def plugin_help_text():
    banner("CROS Plugins", "See what plugin tools CROS can load and how plugin releases are installed")    print(color("To view plugin information:", "CYAN"))
                ...
                if callable(panel_builder):
                    ...
                    if result is not None:
                        try:
                            ...
                            pass                    self.log_line(f"Opened plugin panel: {plugin.get('name', plugin.get('id'))}","ok")
                    return
            ...
            def current_matches():
                ...
                for b in state["catalog"]:
                    ...
                    if score is not None:
                        scored.append((score,b))                scored.sort(key=lambda item:(-item[0], item[1]["name"].casefold()))
...
cat > "$BIN_DIR/cros" <<'SH'#!/usr/bin/env bash
set -e
